...
class MambaOut(nn.Module):
    ...
    def forward_features(self, x):
        ...
        return outputs#self.norm(x.mean([1, 2])) # (B, H, W, C) -> (B, C)
...
class mamba_tim(nn.Module):
    def __init__(self):
        super(mamba_tim, self).__init__()  # Ensure nn.Module is properly initialized
        ...
        self.backbone .default_cfg = default_cfgs['mambaout_small']
        if pretrained:
            state_dict = torch.hub.load_state_dict_from_url(
                url=  self.backbone.default_cfg['url'], map_location="cpu", check_hash=True)
    ...
    def forward(self, x):
        ...
        # print("Size of extracted features in stage 0:", features[0].size())
        # print("Size of extracted features in stage 1:", features[1].size())

        # print("Size of extracted features in stage 2:", features[2].size())
        # print("Size of extracted features in stage 3:", features[3].size())

        return features

@MODELS.register_module()
class mamba_small_timm(mamba_tim):
    def __init__(self, **kwargs):
        super(mamba_small_timm, self).__init__()
