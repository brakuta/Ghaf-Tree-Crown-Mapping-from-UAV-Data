...
To ship the models' output alongside the weights:

```bash
python tools/predict_split.py \
    configs/ghaf/fastvit-ma36_mask2former.py \
    ghaf-release/fastvit-ma36_mask2former/best_mIoU_iter_3500.pth \
    --data-root data/ghaf --split testing \
    --out-dir ghaf-release/predictions/testing --save-probability
```

Each predicted mask carries its source tile's CRS and geotransform and uses the
same encoding as the ground truth, so a recipient can open both in GIS and
difference them directly. A `summary.json` records the tile count, threshold
and canopy fraction.
